...
<div class="container grid grid-cols-1 md:grid-cols-3 gap-5 justify-items-center justify-center bg-white py-6 border-b border-gray-300 fixed">
    ...
    <div class="ml-48 p-1">
        <label for="simple-search" class=" text-2xl font-bold text-black ">  الأشخاص الكفيفين </label>

    </div>

    <div class="flex items-center max-w-sm mx-auto mb-4">
        <form action="{{ route('search.blind') }}" method="GET" class="flex w-full">
            <input type="text" name="query" class="w-80 bg-green-50 border border-customGreen text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-customGreen block p-2.5" placeholder="ابحث عن الكفيفين بالاسم، المدينة" />
...
<div class="grid  grid-cols-1 md:grid-cols-2  gap-8 md:py-36 py-60  mx-8 ">
    ...
    <div class="container grid grid-cols-1 md:grid-cols-2 mx-auto p-8 bg-white rounded-2xl ">
        ...
        <div class="col-span-2 flex justify-center justify-items-center mt-4">
            <a href="{{ route('blind.profile', Crypt::encrypt($blind->id)) }}" class="text-center py-2 bg-customGreen text-white border border-customGreen hover:bg-white hover:text-customGreen rounded-lg font-bold text-sm w-full h-10">عرض الملف الشخصي</a>
