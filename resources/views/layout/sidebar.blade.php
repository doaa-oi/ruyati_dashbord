<aside id="logo-sidebar" class="fixed top-0 right-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
       <ul class="space-y-2 font-medium">
          <li>
           <a tabindex="0" class="navigable flex items-center flex-shrink-0 h-14 px-2 text-sm font-medium rounded hover:bg-customGreen hover:text-white {{ Request::is('blinds') ? 'active' : 'bg-white text-customGreen' }}" href="{{ route('blinds.index') }}">
          ...
          <li>
           <a tabindex="0" class="navigable flex items-center flex-shrink-0 h-14 px-2 text-sm font-medium rounded hover:bg-customGreen hover:text-white {{ Request::is('profileBlind') ? 'active' : 'bg-white text-customGreen' }}" href="{{ route('blinds.profile') }}">
          ...
          <li>
           <a tabindex="0" class="navigable flex items-center flex-shrink-0 h-14 px-2 text-sm font-medium rounded hover:bg-customGreen hover:text-white {{ Request::is('my-help-request') ? 'active' : 'bg-white text-customGreen' }}" href="{{ route('my.help.request') }}">
               ...
               <span class="mr-4 leading-none">طلباتي</span>
           </a>
          </li>

          <li>
             <a tabindex="0" class="navigable flex items-center flex-shrink-0 h-14 px-3 mt-96 text-sm font-medium rounded hover:bg-customGreen hover:text-white {{ Request::is('logout') ? 'active' : 'bg-white text-customGreen' }}" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
